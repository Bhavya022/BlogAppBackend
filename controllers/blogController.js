...
exports.commentOnBlog = async (req, res) => {
  try {
    ...
    if (!updatedBlog) {
      return res.status(404).json({ error: 'Blog not found for comment update' });
    }
...
exports.searchBlogs = async (req, res) => {
  try {
    ...
    const searchedBlogs = await Blog.find({ title: { $regex: query, $options: 'i' } });
...
exports.filterAndSortBlogs = async (req, res) => {
  try {
    ...
    const filteredAndSortedBlogs = await Blog.find({ category }).sort({ date: order });
